rule RT_coldspot_df:
    ...
    output:
        df="results/{dset}/rates/coldspot_df.csv",
    conda:
...
rule RT_terminal_coldspots:
    ...
    output:
        cdf="results/{dset}/rates/terminal_coldspot.csv",
        edf="results/{dset}/rates/terminal_branches.csv",
    conda:
        "../conda_env/bioinfo.yml"
    shell:
        """
...
rule RT_internal_coldspots_prepare:
    input:
        ...
        jpg=all_junction_pangraphs,
    output:
        ABs="results/{dset}/rates/AB_states.csv",
        ev="results/{dset}/rates/AB_nestedness.csv",
    conda:
        "../conda_env/bioinfo.yml"
    shell:
        """
...
rule RT_internal_coldspots_mugration:
    input:
        ...
        tree=rules.PG_filtered_coregenome_tree.output.nwk,
    output:
        jdf="results/{dset}/rates/nonsingleton_junct_df.csv",
        bdf="results/{dset}/rates/nonsingleton_branch_df.csv",
    conda:
...
rule RT_events_df:
    ...
    output:
        df="results/{dset}/rates/merged_events_df.csv",
    conda:
        "../conda_env/bioinfo.yml"
    shell:
        """
...
rule FG_junctions_ann:
    ...
    output:
        ff=directory("figs/{dset}/junctions/"),
    conda:
        "../conda_env/bioinfo.yml"
    shell:
        """
...
rule FG_rates:
    ...
    output:
        ff=directory("figs/{dset}/rates"),
    conda:
        "../conda_env/bioinfo.yml"
    shell:
        """
        python3 scripts/rates/rates_figs.py \
...
rule RT_all:
    input:
        expand(rules.RT_events_df.output, dset=dset_names),
