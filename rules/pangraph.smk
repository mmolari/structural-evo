...
rule PG_export:
    ...
    output:
        directory("results/{dset}/pangraph/export"),
    conda:
        "../conda_env/pangraph.yml"
    shell:
        """
...
rule PG_corealignment:
    input:
        rules.PG_polish.output,
    output:
        fa="results/{dset}/pangraph/alignment/corealignment.fa",
        json="results/{dset}/pangraph/alignment/corealignment_info.json",
    conda:
...
rule PG_filtered_corealignment:
    input:
        rules.PG_polish.output,
    output:
        fa="results/{dset}/pangraph/alignment/filtered_corealignment.fa",
        info_size="results/{dset}/pangraph/alignment/filtered_corealignment_info_size.json",
        info_idxs="results/{dset}/pangraph/alignment/filtered_corealignment_info_idxs.json",
    conda:
        "../conda_env/bioinfo.yml"
    params:
        window=1000,
...
rule PG_coregenome_tree:
    ...
    output:
        nwk="results/{dset}/pangraph/raw_coretree.nwk",
    conda:
...
rule PG_filtered_coregenome_tree:
    ...
    output:
        nwk="results/{dset}/pangraph/coretree.nwk",
    conda:
        "../conda_env/tree_inference.yml"
    params:
...
rule PG_all:
    input:
        expand(rules.PG_coregenome_tree.output, dset=dset_names),
        expand(rules.PG_filtered_coregenome_tree.output, dset=dset_names),
        expand(rules.PG_export.output, dset=dset_names),
